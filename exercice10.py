...
# afficher les employés qui ont vendu le plus de cocktails du jour
print("\nVoici la liste des 5 employés qui ont vendu le plus de cocktails du jour.")
...
results = curseur.fetchall()
for r in results :
    print(f"{r[1]} {r[0]} a vendu {r[2]} cocktail du jour.")

# afficher les employés qui ont vendu le plus de bières en pression.
print("\nVoici la liste des 5 employés qui ont vendu le plus de bières en pression.")
curseur.execute(f"SELECT E.nom, E.prenom, COUNT(V.idBoisson) \
            FROM Employes AS E \
            INNER JOIN Ventes AS V \
            ON E.matricule = V.matricule \
            ...
            ON C.idBoisson = V.idBoisson \
            WHERE E.nom_bar = \"{nom_bar[0]}\" \
            AND boisson = \"Blonde pression\" \
            GROUP BY V.matricule \
            ORDER BY COUNT(V.idBoisson) DESC \
            LIMIT 5")
results = curseur.fetchall()
for r in results :
    print(f"{r[1]} {r[0]} a vendu {r[2]} bières en pression.")